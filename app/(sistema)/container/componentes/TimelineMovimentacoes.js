...
export default function TimelineMovimentacoes({
  ...
  // 🔥 NOVO: GERA URL DINÂMICA
  async function abrirPreview(mov) {
    try {
      let url = mov.arquivo_url

      // 👉 Se já for URL antiga (compatibilidade)
      if (url.startsWith("http")) {
        setPreview(url)
        return
      }

      // 👉 Se for novo padrão (fileName)
      const { data, error } = await supabase.storage
        .from("guias-humanitarias")
        .createSignedUrl(url, 60 * 60) // 1 hora
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-sm">
                      ...
                      <span className="text-slate-500">
                        Guia: <span className="text-slate-700 font-mono">{mov.guia}</span>
                      </span>
                    </div>
