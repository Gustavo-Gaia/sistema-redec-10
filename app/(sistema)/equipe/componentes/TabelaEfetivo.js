...
export default function TabelaEfetivo({ militares, onEdit }) {
  ...
  // 🔧 Função para tratar RG como número para ordenação
  const limparRG = (rg) => {
    if (!rg) return 9999999;
    return parseInt(rg.toString().replace(/\D/g, ""), 10);
  };

  // 🔥 CORREÇÃO GLOBAL DE DATA (anti bug UTC)
      ...
      <div className="overflow-x-auto">
                ...
                <tr key={m.id} className="hover:bg-blue-50/40 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className={`font-bold leading-none mb-1 uppercase text-sm ${
                  ...
                  <td className="px-6 py-4">
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                      ...
                      <p className="text-[11px] font-medium text-slate-600">
                        <span className="text-[9px] font-black text-slate-300 uppercase mr-1">RG:</span>
                        {m.rg || '---'}
                      </p>

                      <p className="text-[11px] font-medium text-slate-600">
                        <span className="text-[9px] font-black text-slate-300 uppercase mr-1">ID:</span>
                        {m.id_funcional || '---'}
                      </p>

                      <p className="text-[11px] font-medium text-slate-600 col-span-2">
                        <span className="text-[9px] font-black text-slate-300 uppercase mr-1">CPF:</span>
                        {m.cpf || '---'}
                      </p>
                    ...
                    <div className="flex flex-col items-center gap-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <p className="text-[10px] font-bold text-slate-400 flex items-center gap-1">
                          ...
                          {formatarDataBR(m.data_entrada_redec)}
                        </p>

                        {!ativo && m.data_saida_redec && (
                          <p className="text-[10px] font-bold text-slate-400 flex items-center gap-1">
                            <UserMinus className="w-3 h-3 text-rose-500" />
                            {formatarDataBR(m.data_saida_redec)}
                          </p>
                        )}
        ...
        {militaresProcessados.length === 0 && (
          <div className="p-24 text-center">
            ...
            <p className="text-slate-400 text-[10px] font-medium mt-1 uppercase">
              Ajuste os filtros ou a busca para encontrar militares.
            </p>
          </div>
        )}
      </div>
